...
for marker_id in range(num_markers):
    ...
    # Convert the marker to green (change black regions to green)
    green_marker = np.zeros((marker_size, marker_size, 3), dtype=np.uint8)
    green_marker[marker_image == 0] = [0, 255, 0]  # Set black pixels to green
    green_marker[marker_image == 255] = [255, 255, 255]  # Set white pixels to white
    ...
    # Place the green marker on the new image (centered)
    bordered_marker[white_boundary_size:white_boundary_size + marker_size, white_boundary_size:white_boundary_size + marker_size] = green_marker
...
print(f"Generated {num_markers} green markers with a {white_boundary_size}px white boundary and a {black_border_size}px black border, and saved them in '{output_dir}' directory.")
